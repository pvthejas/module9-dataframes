...
write.csv(husky.games.2015, "huskies_2015_improvised.csv")


# What is the score of the game where the Huskies had the most combined yards?
husky.max.combined <- husky.games.2015[max(combined), uw_score]
...
MostYardsScore <- function(games) {
  games.max.combined <- games[combined == max(combined), uw_score]
  paste("The game with the most rushing yards had", max(combined), "with a max score of", games.max.combined)
}
